...
int main(){
    ...
    int test_data[] = {13, 27, -5, 42, -18, 99, 0, -77, 56, 33, -91, 2};
    ...
    // int test_data[] = {1, 2, EMPTY, 4, 5, EMPTY, EMPTY, 8, 9, EMPTY, EMPTY, 12};
    ...
    size_t data_size1 = sizeof(test_data_A) / sizeof(test_data_A[0]);
    if (data_size1 != rows1 * cols1){
        printf("error size");
        cleanMatrix(matrix11, rows1);
        matrix11 = NULL;
        cleanMatrix(matrix22, rows2);
    ...
    size_t data_size2 = sizeof(test_data_B) / sizeof(test_data_B[0]);
    if (data_size2 != rows2 * cols2){
        printf("error size");
        cleanMatrix(matrix11, rows1);
        matrix11 = NULL;
        cleanMatrix(matrix22, rows2);
    ...
    free(matrix11[1]);      // освобождаем вторую строку
    matrix11[1] = NULL;
    ...
    for (size_t i = 0; i < rows1; i++) {
        ...
        for (size_t j = 0; j < cols2; j++) {
            ...
            for (size_t k = 0; k < cols1; k++) {
                // проверяем, что строка второй матрицы не NULL
                if (matrix22[k] == NULL) {
                    continue;
                }
